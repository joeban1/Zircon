...
namespace MirBot
{
    ...
    public static class ConfigSchema
    {
        private static readonly List<ConfigField> Fields = new List<ConfigField>
        {
            ...
            B("NotifyLevelUp", "Notify", "phone alert on a level-up"),
            B("NotifyUpgrade", "Notify", "phone alert on a confirmed gear upgrade"),
            B("NotifySkill", "Notify", "phone alert when a skill is learned"),
            B("NotifyDeath", "Notify", "phone alert on death (noisy)"),
            B("NotifyFault", "Notify", "phone alert when a bot faults and stops"),
            B("NotifyQuest", "Notify", "phone alert when a quest is completed"),
            B("NotifyFame", "Notify", "phone alert when a fame rank is reached"),
            B("NotifyCombine", "Notify", "phone alert when a combination (Payton) succeeds or fails"),
            B("EnableQuests", "Quests", "take and complete the whitelisted NPCs' kill quests"),
            F("QuestBossMinLevel", 1, 250, "Quests", "level for a heavy mini-boss target (Crazed Warrior)"),
            F("QuestMiniBossMinLevel", 1, 250, "Quests", "level from which a quest whose target is a mini-boss is taken"),
            F("QuestHuntChancePercent", 0, 100, "Quests", "chance a hunting choice pursues accepted quests"),
            F("MaxConsecutiveQuestHunts", 1, 10, "Quests", "quest-goal choices before another goal is forced"),
            B("EnableFame", "Quests", "buy fame ranks from the fame NPC when affordable"),
            B("EnableCombine", "Quests", "bank Rusty/Cracked/Worn pieces and combine a full set at Payton"),
            B("EnableStore", "Store", "buy the class's game store list with Hunt Gold and use it"),
            F("StoreRebuyMinutes", 5, 1440, "Store", "rebuy a temporary store buff when this little is left"),
            F("NotifyIdleMinutes", 0, 600, "Notify", "phone alert after this many minutes with no XP (0 = off)"),
            ...
            F("AoeMinimumTargets", 1, 10, "Combat", "distinct hostiles required before an area cast (classes without their own setting)"),
            F("AoeMinimumTargetsWizard", 1, 10, "Combat", "Wizard: distinct hostiles required before an area cast"),
            ...
            F("FightThroughSeconds", 0, 600, "Travel", "seconds of fighting without a kill before the stall watchdog resumes"),
            F("FightThroughMaxSeconds", 0, 3600, "Travel", "hard ceiling on one fight-through, however well it is going"),
            F("JourneyRetries", 0, 10, "Travel", "times an abandoned journey is resumed before choosing afresh"),
            F("JourneyRetryWindowMinutes", 1, 1440, "Travel", "minutes a journey failure counts against the retries"),
            F("JourneyLootMinValue", 0, 10000000, "Loot", "while travelling, ordinary drops must sell for this much (not when poor)"),
